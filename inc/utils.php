...
* @since 1.0.1
*/
function sharing_plus_fetch_shares_via_curl_multi( $data, $options = array() ) {
  ...
  * @since 1.0.1
  */
  function sharing_plus_fetch_fresh_counts( $stats, $post_id ,$alt_share_link) {
    ...
    if( 0 !== $post_id ){
      $networks = get_post_meta( $post_id, 'sharing_plus_old_counts', true );
    }else{
      $networks = get_option(  'sharing_plus_not_exist_post_old_counts' );
    }

    if( ! $networks ){
      $_result = sharing_plus_fetch_shares_via_curl_multi( array_filter( $alt_share_link ) );
      sharing_plus_fetch_http_or_https_counts( $_result, $post_id );
      // special case if post id not exist for example short code run on widget out side the loop in archive page
      if( 0 !== $post_id ){
        $networks = get_post_meta( $post_id, 'sharing_plus_old_counts', true );
      }else{
    ...
    foreach ( $stats as $social_name => $counts ) {
      ...
      $old_counts = get_post_meta( $post_id, 'sharing_plus_' . $social_name . '_counts', true );
      ...
      if( 0 !== $post_id ) {
        if ( $new_counts > $old_counts ) {
          update_post_meta( $post_id, 'sharing_plus_' . $social_name . '_counts', $new_counts );
        } else {
          // set new counts = old counts for total calculation.
          $new_counts = $old_counts;
        }
      } else {
        update_option( 'sharing_plus_not_exist_post_'. $social_name .'_counts', $new_counts );
      }
    ...
    // special case if post id not exist for example short code run on widget out side the loop in archive page
    if( 0 !== $post_id ){
        update_post_meta( $post_id, 'sharing_plus_total_counts', $total );
    }else{
    update_option( 'sharing_plus_not_exist_post_total_counts', $total );
    }
...
/**
   ...
   * @since 1.0.1
   */
  function sharing_plus_whats_app_share_link( $url ){
    $whats_share_link = '';
    if( wp_is_mobile() ){
